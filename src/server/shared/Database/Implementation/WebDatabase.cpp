...
void WebDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(WEB_SEL_ACCOUNT_POINTS, "SELECT Points FROM `api_points` WHERE `AccountID` = ?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(WEB_UDP_ACCOUNT_POINTS, "UPDATE `api_points` SET Points = Points - ? WHERE `AccountID` = ?", CONNECTION_ASYNC);
}
